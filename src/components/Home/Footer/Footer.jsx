...
function FooterBlock() {
  ...
  function ListItemsAll() {
    return baadiList.map((lst, index) => {
          return (
            <div className="col_ col-span-2 sm:col-span-1"
              // style={{ gridArea: `col_${index}` }}
              key={index}
              data-footer-part=""
            >
                <h3 onClick={() => prev !== index + 1 ? cngView(index + 1) : cngView(0) } className="drpBtn font-medium text-gray-900">
                  {lst.title}
                  {toggleView === index + 1 ? (
                ...
                <ul className={`close ${toggleView === index + 1 ? "all_Items" : "" } mt-6 space-y-3 text-sm`}>
                  {lst.chhotiList.map((list2, int) => {
                    return (
                      <li key={int}>
                        <Link to={list2.url} className="footerList text-gray-700 transition hover:text-pink-400">
                          {list2.txt}
                        </Link>
                      </li>
                    );
  ...
  return (
    <footer data-footer-part="">
      <div className="mx-auto max-w-screen-xl space-y-8 px-8 py-16 sm:px-28 md:px-40 lg:px-8 lg:pt-16">
        <div className="row_ mt-8 grid grid-cols-1 lg:mt-0 lg:grid-cols-5" data-footer-part="">
          <div className="col_ col-span-2 sm:col-span-1 lg:col-span-1"
          ...
          <ul className="col_ col-span-2 flex justify-center gap-6 pt-4 md:pt-0 lg:pt-0 lg:pr-32 lg:col-span-5 lg:justify-end">
            <li>
              <a href="" className="text-gray-700 transition hover:text-pink-400">
        ...
        <div className="pt-4">
          <div className="sm:flex sm:justify-between">
            ...
            <ul className="mt-8 flex flex-wrap justify-center gap-4 text-xs sm:mt-0 lg:justify-end">
              <li>
                <a href="/" className="text-gray-500 transition hover:text-pink-400"> Terms & Conditions </a>
              </li>

              <li>
                <a href="/" className="text-gray-500 transition hover:text-pink-400"> Privacy Policy </a>
              </li>

              <li>
                <a href="/" className="text-gray-500 transition hover:text-pink-400"> Cookies </a>
              </li>
            </ul>
          </div>
        </div>
